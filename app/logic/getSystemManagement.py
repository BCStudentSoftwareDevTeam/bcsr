...
class GetSystemManagement():
  ...
  def get_years_list(self):
    ...
    def create_seid(self, data):
      ...
      for x in same_year:
        if int(term_key) == x:
          term_code == year + term_key
        else:
          term_code == str(int(year)-1) + term_key
      return term_code
    ...
    def add_semester(self, data):
      ...
      checkSeid     = Semesters.select().where(Semester.SEID == SEID)
      term_name = cfg['termInfo'][data['term']]
      if checkSeid:
        #TODO: Log error
        ...
        try:
          newSemester = Semester(SEID = int(SEID),
                                 term = term_name,
                                 year = int(data['year']))
